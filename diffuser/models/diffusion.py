...
class GaussianInvDynDiffusion(nn.Module):
    def __init__(self, model, horizon, observation_dim, action_dim, n_timesteps=1000,
        ...
        condition_guidance_w=0.1, ar_inv=False, train_CDMP_pen=False):
        super().__init__()
        ...
        self.model = model
        self.ar_inv = ar_inv
        self.train_CDMP_pen = train_CDMP_pen
        self.ood_weight = 0.1
        self.inv_weight = 0.5
        if self.ar_inv:
    ...
    def p_losses(self, x_start, cond, t, returns=None):
        ...
        epsilon_pred = self.model(x_noisy, cond, t, returns)     
        x_recon = self.predict_start_from_noise(x_noisy, t=t, noise=epsilon_pred)  

        x_recon = apply_conditioning(x_recon, cond, 0)

        assert noise.shape == epsilon_pred.shape

        if self.predict_epsilon:
            loss, info = self.loss_fn(epsilon_pred, noise)
        else:
            loss, info = self.loss_fn(epsilon_pred, x_start)

        return loss, info, x_recon

    def loss(self, x, cond, returns=None):
        if self.train_CDMP_pen:
            batch_size = len(x)
            t = torch.randint(0, self.n_timesteps, (batch_size,), device=x.device).long()
            diffuse_loss, info, x_recon = self.p_losses(x[:, :, self.action_dim:], cond, t, returns)
            ...
            # OOD penalty term (starting from t=2)
            x_start = x[:, :, self.action_dim:]

            
            x_recon_clipped = torch.clamp(x_recon, min=-1, max=1)    
            ood_loss = ((x_start[:, 1:, :] - x_recon_clipped[:, 1:, :]) ** 2).mean()


            # Total loss
            total_loss = diffuse_loss + self.inv_weight * inv_loss + self.ood_weight * ood_loss
            info['diffuse_loss'] = diffuse_loss.item()
            info['inv_loss'] = inv_loss.item()
            info['ood_loss'] = ood_loss.item()
            ...
            loss = (diffuse_loss + 0.5 * inv_loss) / 2
...
class ARInvModel(nn.Module):
    def __init__(self, hidden_dim, observation_dim, action_dim, low_act=0.0, up_act=23.0):
        ...
        self.bin_size = (self.up_act - self.low_act) / (self.num_bins - 1)
    ...
    def forward(self, comb_state, deterministic=True):
        ...
        if deterministic:
            ...
            a_0 = self.low_act + l_0 * self.bin_size
        else:
        ...
        for i in range(1, self.action_dim):
            ...
            if deterministic:
                ...
                a_i = self.low_act + l_i * self.bin_size
            else:
    ...
    def calc_loss(self, comb_state, action):
        ...
        l_action = torch.div((action - self.low_act + self.bin_size/2), self.bin_size, rounding_mode='floor').long()
